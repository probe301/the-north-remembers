...
class Collector:
  ...
  def report(self):
    ...
    log(f'report {self} \n---------------------')
    for directory in self.iter_watchers():
  ...
  def find_watcher(self, folder_name):
    for directory in self.iter_watchers():
  ...
  def generate_feed(self, path, limit, site='http://xxx.com/'):
    ''' 生成 RSS, 内容为全部 page_task, 
        ...
        path 如果是父级文件夹, 视为合并子文件夹里所有 page 生成 rss feed
        RSS Feed 文件名为 path, alias? TODO

        limit=-1 时迭代所有Page
        '''
    watcher_path = self.find_watcher(path)
    ...
    fg.link(href=site + feed_name + '.atom', rel='self')
    fg.language('zh-cn')
