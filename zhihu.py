...
def _answer_content_process(content):
    ...
    img_list = soup.find_all(
        # name="img", class_="origin_image zh-lightbox-thumb lazy")
        name="img", class_="lazy")
        # 发现之后大量图片没有class origin_image zh-lightbox-thumb
        # 只有 class lazy
        # 如 <img class="content_image lazy" data-actualsrc="http://pic3.zhimg.com/aff781f2ea4bf8bf2c9ab4ef441b8bee_b.jpg" data-rawheight="311" data-rawwidth="209" src="//s1.zhimg.com/misc/whitedot.jpg" width="209">
    for img in img_list:
        # print(img)
...
class Question:
    ...
    def details(self):
        ...
        **目前实现方法只是直接获取文本，效果不满意……等更新**
        ...
        :rtype: str
        """
        full_detail = self.soup.find('textarea', class_="content hidden")
        if full_detail:
            return full_detail.text
        else:
            return self.soup.find("div", id="zh-question-detail").div.prettify()
...
class Author():
    ...
    def answers(self):
        ...
        for page_index in range(1, (self.answers_num - 1) // 20 + 2):
            global _session
            html = _session.get(
                self._url + 'answers?order_by=vote_num&page=' + str(page_index)).text
            soup = BeautifulSoup(html)
...
class Answer():
    ...
    def question(self):
        ...
        # 不知道怎么回事无法取得侧栏
        # print(title)
        # print(self.soup.find('div', class_='zh-question-followers-sidebar'))
        followers_num = _text2int(self.soup.find('div', class_='zh-question-followers-sidebar').div.a.strong.text)
        answers_num = _text2int(_re_get_number.match(self.soup.find('div', class_='zh-answers-title').h3.a.text).group(1))
        # followers_num = answers_num = 0
    ...
    def comments(self):
        ...
        for div in soup.find_all('div', class_='zm-item-comment'):
            ...
            comment = Comment(raw_content, comment_id, author, likes, content, reply_to)
...
class Topic():
    ...
    def __init__(self, url, name=None):
        if url.isdigit():
            url = 'http://www.zhihu.com/topic/{}/top-answers'.format(url)
        elif url[-1] == '/':
            url += 'top-answers'
        elif url[-1].isdigit():
            url += '/top-answers'
    ...
    @_check_soup('_name')
    def name(self):
        return self.soup.find('div', id='zh-topic-title').find('h1').text.strip()

    @property
    @_check_soup('_page_number')
    def page_number(self):
        pager = self.soup.find('div', class_='zm-invite-pager')
        if pager:
            numbers = [int(a.text) for a in pager.find_all('a')[:-1]]
            return max(numbers)
        else:
            return 1

    @property
    @_check_soup('_top_answers')
    def top_answers(self):
        for page in range(1, self.page_number+1):
            if page == 1:
                soup = self.soup
            else:
                url = self._url + '?page={}'.format(page)
                soup = BeautifulSoup(_session.get(url).content)

            for content in soup.find_all('div', class_='feed-item'):
                # question_url = content.find('a', class_='question_link').href
                question_title = content.find('a', class_='question_link').text

                body = content.find('textarea', class_='content hidden').contents

                # print(body)
                pattern = re.compile(r'href="(/question/\d+/answer/\d+)"')
                # print(str(body[-2]))
                answer_url = pattern.search(str(body[-2])).group(1)

                upvote = int(content.find('a', class_='zm-item-vote-count')['data-votecount'])
                from_sub_topic = content.find('div', class_='subtopic')
                if from_sub_topic:
                    from_sub_topic = from_sub_topic.find('a').text.strip()

                answer = Answer(_Zhihu_URL+answer_url, upvote=upvote)
                yield answer, upvote, question_title, from_sub_topic
            ...
            for tag in answer_tags:
                ...
                if h3.text != '匿名用户':
                    ...
                    if h3.strong is not None:
                        author_motto = h3.strong['title']
                # print('create', author_name, author_motto)
